...
// Helper function to implement retry logic for Supabase queries
const executeWithRetry = async <T>(queryFn: () => Promise<T>, maxRetries = 3, delay = 1000): Promise<T> => {
  let retries = 0;
  while (true) {
    try {
...
export const checklistService = {
  ...
  createChecklist: async (checklist: Checklist) => {
    try {
      ...
      // Insert the checklist with retry logic
  ...
  saveChecklist: async (checklistId: string) => {
    try {
      ...
      const numericId = parseInt(checklistId);
      if (isNaN(numericId)) {
        throw new Error(`Invalid checklist ID: ${checklistId}`);
      }
    ...
    try {
      ...
      const response = await executeWithRetry(async () => {
        return await supabase
          ...
          .limit(10); // Limit results to avoid resource errors
      });
      
      const { data: dbChecklists, error } = response;
    ...
    try {
      ...
      const response = await executeWithRetry(async () => {
        return await supabase
          ...
          .select('id, colaborador_id, data, vistoria1, vistoria2, vistoria3, status_vistoria1, status_vistoria2, status_vistoria3, setor_id, unidade, observacoes, tipo')
          .eq('data', date)
          .limit(10); // Lower limit to reduce resource usage
      });
      ...
      for (const checklist of dbChecklists) {
        ...
        // Only create checklists for non-null vistoria data
        if (checklistData.vistoria1) {
          appChecklists.push(
            mapDatabaseChecklistToAppChecklist(checklistData, "vistoria1" as ChecklistType, "manhã")
          );
    ...
    try {
      ...
      const response = await executeWithRetry(async () => {
        return await supabase
          .from('checklists')
          .select('*')
          .limit(10); // Add limit to reduce resource usage
      });
...
export const actionPlanService = {
  ...
  addActionPlan: async (actionPlan: Omit<ActionPlan, 'id'>) => {
    try {
      ...
      if (error) throw error;
      if (!data) throw new Error("No data returned after inserting action plan");
  ...
  getPendingActionPlans: async () => {
    try {
      // Simplify query to reduce resource usage - avoid using join
      const response = await executeWithRetry(async () => {
        return await supabase
          .from('planos_acao')
          .select('*')
      ...
      // Format the data with default values when info is missing
      return data.map(plan => {
        const createdAt = new Date(plan.data_envio || new Date());
        const now = new Date();
        const daysPending = Math.floor((now.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24));

        return {
          id: String(plan.id),
          description: plan.descricao || "Plano de Ação",
          createdAt: plan.data_envio || now.toISOString().split('T')[0],
          daysPending: daysPending || 1,
          storeId: String(plan.checklist_id || "unknown"),
          storeName: "Unidade",
          checklistType: "vistoria1" as ChecklistType, 
          itemDescription: "Item do Checklist"
        };
      });
    } catch (error) {
